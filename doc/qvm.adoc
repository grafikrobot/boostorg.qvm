...
Copyright 2008-2023 Emil Dotchevski. Distributed under the http://www.boost.org/LICENSE_1_0.txt[Boost Software License, Version 1.0].
...
When rotations represented by `a` and `b` are separated by more than 180 degrees, `slerp` always takes the shorter path on the sphere, while `slerp360` does not. There are use cases for both behaviors, since each introduces a discontinuity under different conditions.
